...
const salesLastChoiceLeftAligned = await page.locator("#salesQuestionnaireView .sales-long-choice button").first().evaluate(element => getComputedStyle(element).textAlign === "left" ? 1 : 0);
...
await page.getByRole("button", { name: "授权销售" }).click();
const salesAuthVisible = await page.locator("#salesAuthView:not(.hidden)").count();
const salesAuthActiveNav = await page.locator(".nav-item.active").innerText();
const salesAuthNoteVisible = await page.locator('.note-card[data-note="sales-auth"]:not(.hidden)').count();
const salesAuthOtherNotesVisible = await page.locator('.note-card:not([data-note="sales-auth"]):not(.hidden)').count();
const salesAuthScreenshotVisible = await page.locator('#salesAuthView img[src="assets/sales-auth-user-list.png"]').count();
await page.screenshot({ path: path.join(validationDir, "18-sales-auth.png"), fullPage: true });

await page.getByRole("button", { name: "App 金刚区入口" }).click();
const appEntryVisible = await page.locator("#appEntryView:not(.hidden)").count();
const appEntryActiveNav = await page.locator(".nav-item.active").innerText();
const appEntryNoteVisible = await page.locator('.note-card[data-note="app-entry"]:not(.hidden)').count();
const appEntryOtherNotesVisible = await page.locator('.note-card:not([data-note="app-entry"]):not(.hidden)').count();
const appEntryLabelVisible = await page.locator("#appEntryView .intake-entry", { hasText: "患教对接" }).count();
await page.screenshot({ path: path.join(validationDir, "19-app-entry.png"), fullPage: true });
...
const salesAppDeleteSheetCentered = await page.locator("#appDeleteSheet:not(.hidden) .app-confirm-card").evaluate(card => {
  const sheet = card.closest("#appDeleteSheet");
  const sheetRect = sheet.getBoundingClientRect();
  const cardRect = card.getBoundingClientRect();
  const sheetCenterY = sheetRect.top + sheetRect.height / 2;
  const cardCenterY = cardRect.top + cardRect.height / 2;
  return Math.abs(sheetCenterY - cardCenterY) <= 40 ? 1 : 0;
});
...
const systemNoticeSalesFloatVisible = await page.locator("#systemNoticeView #noticeSalesEntry", { hasText: "销售" }).count();
const systemNoticeSalesFloatBadgeVisible = await page.locator("#systemNoticeView #noticeSalesEntry", { hasText: "99+" }).count() + await page.locator("#systemNoticeView #noticeSalesEntry .badge").count() + await page.locator("#systemNoticeView #noticeSalesEntry .red-dot").count();
...
await page.locator('.nav-item[data-view="list"]').click();
...
const enabledReceiveButtons = await page.locator('button[data-action="receive"]:not([disabled])', { hasText: "收案" }).count();
const disabledReceiveButtons = await page.locator('button[data-action="receive"][disabled]', { hasText: "收案" }).count();
...
await page.locator('button[data-action="receive"][data-id="68"]').click();
const prepModalTitle = await page.locator("#modalTitle").innerText();
const reopenedPrepStep = await page.locator("#stepName").innerText();
const receiveStepperLabels = (await page.locator("#stepper .step").allInnerTexts()).map(text => text.replace(/\s+/g, ""));
...
const trialVisiblePrepareSteps = await page.locator('#stepper .step:not(.hidden)').count();
const trialGroupBodyVisible = await page.locator('[data-step-body="2"]:not(.hidden)').count();
const trialFinishButtonVisible = await page.getByRole("button", { name: "完成收案" }).count();
...
const processModalTitle = await page.locator("#modalTitle").innerText();
const processStep = await page.locator("#stepName").innerText();
const visiblePrepareStepsInProcess = await page.locator('#stepper .step:not(.hidden)').count();
...
console.log(JSON.stringify({
  ok: consoleErrors.length === 0 && failedRequests.length === 0 && initialTableRows >= 8 && initialTableScrollLeft === 0 && firstIdHeaderVisible === true && tableRows >= 8 && salesQuestionnaireVisible === 1 && salesNotesVisible === 1 && salesNoteVisible === 1 && salesPatientEditableNoteVisible === 1 && salesHiddenQuestionNoteVisible === 1 && healthcareNotesVisibleInSales === 0 && salesBrowserTopHidden === 1 && salesAppTopbarHidden === 1 && salesAddedTopicsTitleVisible === 1 && salesAddedTopicsCount === 19 && salesQuestionnaireScrollShellVisible === 1 && salesQuestionnaireShellScrollable === 1 && salesQuestionnaireViewPageSized === 1 && salesAddedTopicsVisibleAfterInternalScroll === 1 && salesQuestionnaireNoteVisibleAfterInternalScroll === 1 && salesAuthVisible === 1 && salesAuthActiveNav === "授权销售" && salesAuthNoteVisible === 1 && salesAuthOtherNotesVisible === 0 && salesAuthScreenshotVisible === 1 && appEntryVisible === 1 && appEntryActiveNav === "App 金刚区入口" && appEntryNoteVisible === 1 && appEntryOtherNotesVisible === 0 && appEntryLabelVisible === 1 && salesAppVisible === 1 && salesAppNoteVisible === 1 && salesAppQuestionnaireHidden === 1 && salesAppHealthcareNotesVisible === 0 && salesAppPhoneVisible === 1 && salesAppUsersTitle === "客户池" && salesAppSummaryCardCount === 0 && salesAppUserTotalVisible === 0 && salesAppSearchFilterVisible === 1 && salesAppSearchPlaceholder === null && salesAppSearchHintVisible === 0 && salesAppSearchButtonVisible === 0 && salesAppSearchMenuOptions === "昵称,ID,电话" && salesAppSearchFilterAfterSelect === "电话" && salesAppSearchMenuHiddenAfterSelect === 1 && salesAppUserIdsVisible === 3 && salesAppUserPhoneRows === 3 && salesAppUserFollowerRows === 0 && salesAppUnreceivedProgress === 1 && salesAppUserCards === 3 && salesAppNewCaseButtons === 3 && salesAppManagementButtons === 3 && salesAppUserDetailPageCount === 0 && salesAppUserActionDescriptions === 0 && salesAppCaseListTitle === "收案管理" && salesAppCaseRecords === 6 && salesAppCaseListFields >= 4 && salesAppCaseListDeleteButtons === salesAppCaseRecords && salesAppCaseListDetailButtons === salesAppCaseRecords && salesAppCaseListEditButtons === salesAppCaseRecords && salesAppCaseStatusesVisible === 6 && salesAppCaseEnabledEditButtons === 3 && salesAppCaseDisabledEditButtons === 3 && salesAppCaseEnabledDeleteButtons === 1 && salesAppCaseDisabledDeleteButtons === 5 && salesAppBackToUsersTitle === "客户池" && salesAppBackToUsersCards === 3 && salesAppEditTitleFromList === "编辑收案" && salesAppBackFromEditTitle === "收案管理" && salesAppDetailTitle === "问卷详情" && salesAppDetailFields >= 2 && salesAppDetailDeleteButtons === 0 && salesAppDetailEditButtons === 0 && salesAppEditTitle === "编辑收案" && salesAppEditFields === 5 && salesAppDeleteTitle === "收案管理" && salesAppDeleteConfirmVisible === 1 && salesAppDeleteSheetCentered === 1 && salesAppDeleteCancelTitle === "收案管理" && salesAppDeleteConfirmHiddenAfterCancel === 1 && salesAppCaseRecordsAfterDelete === salesAppCaseRecords - 1 && salesAppCaseRecordsAfterReset === salesAppCaseRecords && salesAppNewCaseJumpVisible === 1 && salesAppNewCaseJumpActiveNav === "销售部分问卷" && systemNoticeVisible === 1 && systemNoticeActiveNav === "系统通知" && systemNoticeTitle === "系统通知" && systemNoticeMessageVisible === 1 && systemNoticeSalesFloatVisible === 0 && systemNoticeSalesFloatBadgeVisible === 0 && systemNoticeNoteVisible >= 1 && systemNoticeOtherNotesVisible === 0 && systemNoticeSalesAppHidden === 1 && systemNoticeQuestionnaireHidden === 1 && listBrowserTopVisible === 1 && listNotesVisible === 1 && salesNoteHiddenInList === 1 && salesAppNoteHiddenInList === 1 && removedStatusVisible === 0 && noticeNavVisible === 0 && noticeBannerVisible === 0 && stationMailVisible === 0 && pendingDotInitial === 1 && pendingDotTextInitial === "2" && pendingDotAfterAllConfirm === 1 && pendingDotTextAfterAllConfirm === "" && copyActionButtons === tableRows && enabledConfirmButtonsBefore === 2 && enabledConfirmButtonsAfter === 0 && disabledConfirmButtonsAfter === tableRows && firstConfirmedStatus === "已接收" && salesRequiredFields >= 25 && salesSubmitVisible === 1 && salesFormBackground === "rgb(243, 244, 248)" && salesSectionBackground === salesFormBackground && salesLastChoiceLeftAligned === 1 && salesSubmitWarningDemoButtons === 4 && salesSubmitDemoFullyVisible === 1 && salesSubmitWarningTitleVisible === 1 && salesSubmitWarningOneVisible === 1 && salesSubmitWarningClosedByPrimary === 1 && salesSubmitWarningIntensiveVisible === 1 && salesSubmitWarningClosedAfterIntensive === 1 && salesSubmitWarningTwoVisible === 1 && salesSubmitWarningClosedByX === 1 && salesSubmitWarningThreeVisible === 1 && salesSubmitWarningClosedAfterAll === 1 && logisticsFieldVisible === 0 && userNameFieldVisible === 1 && contactFieldVisible === 1 && pumpUserFieldVisible === 1 && glucoseTargetRateVisible === 1 && glucoseReportUploadVisible === 1 && glucoseReportUploadRequired === 0 && fieldNotesVisible === 6 && progressLabelVisible === 0 && mealTimeFieldsVisible === 0 && relatedInfoRequiredMarks === 4 && fieldNotesInsideScrollShell === true && salesFieldOrder.ok === true && phoneModelVisibleInitial === 1 && watchModelHiddenInitial === 1 && watchModelVisibleAfterClick === 1 && phoneModelHiddenAfterClick === 1 && complicationDetailVisible === 1 && complicationOptionVisible === 1 && complicationRequiredMarks === 0 && skinDetailVisible === 1 && skinDetailFieldsVisible === 1 && skinDetailRequiredMarks === 0 && pumpPanelVisibleInitial === 1 && penPanelHiddenInitial === 1 && penPanelVisibleAfterSelect === 1 && pumpPanelHiddenAfterSelect === 1 && penPanelVisibleAfterPremix === 1 && pumpPanelHiddenAfterPremix === 1 && injectionOptions.join(",") === "0次,1次,2次" && injectionRowsInitial === 2 && injectionRowsAfterOne === 1 && injectionRowsAfterZero === 0 && salesNewQuestionSections === 3 && enabledPushButtons === 7 && enabledAutofillButtons === 6 && disabledAutofillButtons === 2 && enabledDeleteButtons === 1 && disabledDeleteButtons === 7 && prepButtons === 0 && processButtons === 0 && enabledReceiveButtons === 7 && disabledReceiveButtons === 1 && finishedActionButtons === 0 && receiveActionButtons === tableRows && autofillSalesSectionVisible === 0 && prepModalTitle === "收案 - 创建群聊" && reopenedPrepStep === "创建群聊" && receiveStepperLabels.join(",") === "1创建看板,2创建群聊,3核对信息" && trialUserControlCount === 0 && prevStepButtonCount === 0 && trialPrepStep === "创建看板" && trialVisiblePrepareSteps === 3 && trialGroupBodyVisible === 0 && trialFinishButtonVisible === 0 && processModalTitle === "收案 - 创建看板" && processStep === "创建看板" && visiblePrepareStepsInProcess === 3 && processStepperHidden === 0 && prepQuestionnaireLinkVisible === 0 && prepQuestionnaireResultsVisible === 2 && processQuestionnaireLinkVisible === 0 && processQuestionnaireResultsVisible === 2 && autofillDrawerHidden === 1,
  initialTableRows,
  ...
  salesAuthVisible,
  salesAuthActiveNav,
  salesAuthNoteVisible,
  salesAuthOtherNotesVisible,
  salesAuthScreenshotVisible,
  appEntryVisible,
  appEntryActiveNav,
  appEntryNoteVisible,
  appEntryOtherNotesVisible,
  appEntryLabelVisible,
  salesAppVisible,
  salesAppNoteVisible,
  ...
  systemNoticeSalesFloatVisible,
  systemNoticeSalesFloatBadgeVisible,
  ...
  screenshots: [
    ...
    "validation/18-sales-auth.png",
    "validation/19-app-entry.png"
  ]
}, null, 2));
