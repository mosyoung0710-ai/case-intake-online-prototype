const initialCases = [
  ...
  { id: 68, name: "mmmoos", sex: "女", age: "0月14天", height: "5.00", weight: "1.00", type: "T1DM（1型糖尿病）", course: "0月14天", plan: "降糖药物治疗", caregiver: "美女", contact: "17722699882", orderDate: "2026-08-06", phone: "17700000568", pump: "丹纳RS", device: "小米", glucoseMonitor: "鱼跃", channel: "天猫", manager: "JJ", status: "待用户填写" },
  ...
  { id: 45, name: "试戴用户", sex: "男", age: "7岁", height: "192.00", weight: "91.00", type: "GDM（妊娠糖尿病）", course: "5年4月", plan: "预混（胰岛素笔）", caregiver: "分公司公司", contact: "18814136001", orderDate: "2026-07-23", phone: "13800041805", pump: "凯联悦泵", device: "OPPO手表", glucoseMonitor: "雅培", channel: "糖糖圈", manager: "JJ", status: "待用户填写", isTrial: true },
...
const topNotice = document.querySelector("#topNotice");
...
const stationMailDialog = document.querySelector("#stationMailDialog");
const stationMailBackdrop = document.querySelector("#stationMailBackdrop");
const stationMailRows = document.querySelector("#stationMailRows");
...
function statusClass(status) {
  ...
  if (status === "待用户填写") return "gray";
  if (status === "用户填写中") return "purple";
  return "";
}

function receiveAction(status) {
  if (["已接收", "待用户填写", "用户填写中"].includes(status)) {
...
function renderRows() {
  rows.innerHTML = cases.map(item => {
    ...
    const canAutofill = ["待用户填写", "用户填写中"].includes(item.status);
...
function relatedMailCases() {
  return cases.filter(item => ["待接收", "已接收"].includes(item.status)).slice(0, 5);
}

function updatePendingNotice() {
  const count = pendingCases().length;
  pendingCount.textContent = count;
  topNotice.classList.toggle("hidden", count === 0);
}

function renderStationMail() {
  stationMailRows.innerHTML = relatedMailCases().map(item => {
    const isPending = item.status === "待接收";
    const statusText = isPending ? "" : "已确认";
    const action = isPending
      ? `<button class="primary-btn compact receive-confirm" data-id="${item.id}">确认</button>`
      : `<span class="mail-status">${statusText}</span>`;
    return `
      <article class="mail-message ${isPending ? "unread" : "confirmed"}">
        <div class="avatar">糖</div>
        <div class="mail-copy">
          <b>新增服务患者提醒：销售推送服务患者【${item.name}】ID：【${item.id}】进入患教，请尽快跟进患者。</b>
          <small>${item.orderDate} 10:${String(item.id).padStart(2, "0").slice(-2)}</small>
        </div>
        ${action}
      </article>
    `;
...
function showCaseManagement() {
  ...
  stationMailDialog.classList.add("hidden");
  stationMailBackdrop.classList.add("hidden");
}

function showStationMail() {
  reviewShell.classList.remove("notes-hidden");
  notesPanel.classList.remove("hidden");
  browserFrame.classList.remove("sales-mode");
  appShell.classList.remove("sales-mode");
  caseManagementView.classList.remove("hidden");
  salesQuestionnaireView.classList.add("hidden");
  stationMailDialog.classList.remove("hidden");
  stationMailBackdrop.classList.remove("hidden");
}
...
function setView(view) {
  ...
  if (view === "notice") {
    showStationMail();
  } else if (view === "sales-questionnaire") {
...
document.querySelector("#openNoticePanel").addEventListener("click", () => setView("notice"));
document.querySelector("#closeStationMail").addEventListener("click", () => setView("list"));
stationMailBackdrop.addEventListener("click", () => setView("list"));
stationMailRows.addEventListener("click", event => {
  const button = event.target.closest(".receive-confirm");
  if (!button) return;
  confirmReceive(button.dataset.id);
});

document.querySelector("#openReceiveFromHeader").addEventListener("click", () => {
  const demoCase = cases.find(item => item.status === "待用户填写") || cases[0];
  openReceiveModal(demoCase.id);
});
